...
interface Props {
    conversation: Types; // 1 conversation đơn
}

    const ConversationItem: React.FC<Props> = React.memo(({ conversation }) => {

        const theme = useTheme();
    const { participantIds, lastMessagePreview, unreadCount, lastUpdated } = conversation;
    ...
    const displayName = otherParticipants.length > 0 ? user[0].username : 'Unknown';

    // Hiển thị avatar participant chính
    const avatarUrl = otherParticipants.length > 0 ? user[0].avatarUrl : undefined;
